...
export function PremiumCard({ planName, statusLabel, dateRangeLabel }: PremiumCardProps) {
  return (
    <Link
      href="/app/subscription"
      className="relative block overflow-hidden rounded-2xl bg-[#1a1a1a] px-6 py-5 shadow-md"
    >
      ...
      <div className="relative">
        <div className="flex items-center gap-3">
          <h2 className="text-[28px] font-bold tracking-tight text-white">{planName}</h2>
        ...
        <p className="mt-3 text-[12px] text-white/55">{dateRangeLabel}</p>
      </div>
    </Link>
  );
}
